...
interface ControlPanelProps {
  ...
  isScanning: boolean;
  onCancel: () => void;
  onSwitchCamera: () => void; // TAMBAHAN: Prop untuk memicu switch dari komponen induk
}
...
export function ControlPanel({
  ...
  isScanning,
  onCancel,
  onSwitchCamera, // Destrukturisasi fungsi prop
}: ControlPanelProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
      ...
      <div className="relative px-6 pb-8 pt-12">
        ...
        <div className="flex items-center justify-center gap-8">
          ...
          <Button
            ...
            aria-label="Switch camera"
            disabled={isScanning}
            onClick={onSwitchCamera} // EKSEKUSI FUNGSI PROP DI SINI
          >
            <SwitchCamera className="h-6 w-6 text-white transition-transform group-hover:scale-110" />
          </Button>
        </div>
